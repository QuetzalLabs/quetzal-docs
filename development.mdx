...
Use the `t` function to translate plain text.
...
export default function Page() {
  ...
  return (
    <p>{t("Here's a {dynamic}", { dynamic: 4 })}</p>
  );
}
```
  ...
  return (
    <p>
      {t(
          ...
          {
              guidelines: (chunks: ReactNode) => (
                  <a href="/guidelines">{chunks}</a>
              ),
          }
      )}
    </p>
...
export default function Page() {
  ...
  return (
    <p>{t(499.9, { style: "currency", currency: "USD" })}</p>
  );
}
```
